...
export default function ServicesPage() {
  ...
  // Fonction pour vérifier si un service est expiré
  const isServiceExpired = (service: Service): boolean => {
    if (!service.active) return true;
    if (service.isLifetime) return false;
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
              {t('dashboard_services_title')}
            </h1>
            <p className="text-gray-600 dark:text-gray-400">
              {t('dashboard_services_desc')}
            </p>
          </div>
          ...
          {services.length === 0 ? (
            <div className="bg-white/70 dark:bg-[#0A0A0A] backdrop-blur-2xl rounded-2xl shadow-lg border border-gray-200/50 dark:border-[#1A1A1A] p-12 text-center">
              ...
              <p className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                {t('dashboard_services_empty_title')}
              </p>
              <p className="text-gray-500 dark:text-gray-400 mb-6">
                {t('dashboard_services_empty_desc')}
              </p>
              <button
              ...
              >
                {t('dashboard_services_view_offers')}
              </button>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {services.map((service) => {
                ...
                const expired = isServiceExpired(service);
                
                // Si le service est expiré, afficher la carte avec overlay
                if (expired) {
                  return (
                    <div
                      key={service.id}
                    ...
                    >
                      ...
                      {!hasCluster && (
                        <div className="absolute -top-3 left-0 right-0 z-20 flex justify-center">
                          <div className="bg-orange-500 dark:bg-orange-600 text-white px-4 py-2 rounded-lg shadow-lg text-sm font-semibold flex items-center gap-2">
                            ...
                            <span>{t('dashboard_services_no_cluster')}</span>
                          </div>
                        </div>
                      )}
                      
                      {/* Carte du service avec opacité réduite */}
                      <div className={`bg-white/70 dark:bg-[#0A0A0A] backdrop-blur-2xl rounded-xl shadow-lg border border-gray-200/50 dark:border-[#1A1A1A] p-4 ${!hasCluster ? 'pt-8' : ''} opacity-50`}>
                        {/* Header */}
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${service.active ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                            <div className="min-w-0 flex-1">
                              <h3 className="text-sm font-bold text-gray-900 dark:text-white transition-colors truncate">
                                {service.name}
                              </h3>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-gray-700 dark:text-gray-300">{service.servers} {service.servers > 1 ? t('dashboard_services_servers_plural') : t('dashboard_services_servers')}</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <Users className="w-3 h-3 text-gray-400" />
                            <span className="text-gray-700 dark:text-gray-300">{service.group} {service.group > 1 ? t('dashboard_services_groups_plural') : t('dashboard_services_groups')}</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <Shield className="w-3 h-3 text-gray-400" />
                            <span className="text-gray-700 dark:text-gray-300">{service.rules} {service.rules > 1 ? t('dashboard_services_rules_plural') : t('dashboard_services_rules')}</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                          ...
                          {service.isLifetime ? (
                            ...
                            <div className="flex flex-col gap-0.5">
                              ...
                              {(() => {
                                ...
                                if (daysLeft < 0) {
                                  return <span className="text-xs text-red-600 dark:text-red-400 font-medium">{t('dashboard_table_expired')}</span>;
                                } else if (daysLeft === 0) {
                                  return <span className="text-xs text-orange-600 dark:text-orange-400 font-medium">{t('dashboard_table_today')}</span>;
                                } else {
                      ...
                      <div className="absolute inset-0 bg-white/60 dark:bg-black/60 backdrop-blur-sm rounded-xl z-10"></div>
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center z-20 p-4">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/dashboard/services/renew/${service.id}`);
                          }}
